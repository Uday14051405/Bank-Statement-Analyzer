    ...
    public function login(Request $request)
    {
        ...
        if ($user) {
            if ($user[0]['password'] == md5($request->password)) {
                ...
                $user[0]['token'] = $token;
                $user1 =  DB::connection('mysql_second')->table('user')->where(['user_name' => $request->user_name, 'status' => 'a', 'type' => 'i'])->whereIn('role_type', ['ibp'])->get(['user_name as hm_username', 'name as hm_name', 'role_type as hm_roletype', 'token as hm_token', 'password as hm_password', 'hm_ibpid as hm_centerid','id as hmid'])->toArray();
                //$data = array_merge($user, $user1);
               ...
               return response(['status' => 200, 'msg' => ['Login Success'], 'data' => $user , 'data1' => $user1]);
