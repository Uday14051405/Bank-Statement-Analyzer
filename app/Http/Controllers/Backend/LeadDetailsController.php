    ...
    public function index(Request $request){
        ...
        if($user == 1){
           ...
           # $headcenter = DB::table('headcenter')->where(['id'=>$request->centerid])->get()->toArray();
            ...
            $total_Sale_nt = DB::table('lead_create_multi')->where([$column=>$request->centerid])->where('ltype', '=', ' ')->orWhereNull('ltype')->where('display_status', '!=', 'N')->orderBy('lead_id', 'DESC')->get(['lead_id','mul_procode','mul_source'])->toArray();
    ...
    public function view(Request $request){
        ...
        if($user == 1){
           ...
           $query = $query->where('mul_proid','=',$proid);
            ...
            $data = $query->get(['l_bid','l_jtitle','l_company','l_tfd','l_itos','l_rs','l_rsl','l_hqc','l_lhs','l_hc','l_pcln','l_pce','l_pcn','l_pcn1','l_pcn2','l_pcn3','l_bid','l_jtitle','purpose','lead_upload_date','ibpid','l_mobile','l_mobile1','l_mobile2','uploadcomment','eadd','adv','prof','id','lead_create_date','l_pc','fdate','l_fname','l_lname','l_pno','l_employee','r_employee','l_rs','source','ibpid','vendor','l_prodt','l_pcfn','Property','l_rsl','ltype','ctype','l_email','fll_details','lead_id'])->toArray();
			#dd($data);
