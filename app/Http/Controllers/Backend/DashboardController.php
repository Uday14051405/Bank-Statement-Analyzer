...
class DashboardController extends Controller
{
  public function index(Request $request)
  {
    ...
    if ($user == 1) {
      ...
      $total_tag_expire = DB::table('temp_lead_cp_app')->where('site_visit_done', '!=', 'Yes')->where('lead_status', '!=', 'LOST')->whereBetween('expiry', [$today, $day_3])->where(['cp_id' => $request->centerid])->count();
      ...
      $arr['micro_site_lead']  = ['count' => $total_micro_site_lead, 'title' => 'Micro Site Lead', 'viewTitle' => 'Micro Site Lead', 'insidedata' => $tag_micro_site_lead];
