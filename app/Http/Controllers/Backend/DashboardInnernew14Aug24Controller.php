...
class DashboardInnernewController extends Controller
{
    ...
    public function view(Request $request)
    {
      ...
      //  $user = DB::table('register')->where(['username' => $request->username, 'id' => $request->centerid, 'usertype' => $request->usertype, 'token' => $request->token])->count();
        $user = DB::table('user')->where(['user_name' => $request->user_name, 'ibpid' => $request->centerid, 'role_type' => $request->role_type, 'token' => $request->token])->count();
        if ($user == 1) {
            ...
            $query = DB::table('npb_lead_create_multi')->where(['ibpid' => $request->centerid])->where('display_status', '!=', 'N');
            $querynew = DB::table('npb_lead_create_multi')->where('display_status', '!=', 'N');
            $querynpb = DB::table('npb_lead_create_multi')->where('display_status', '!=', 'N')->where([$column => $request->centerid]);
            ...
            // }            
            // else{
            $data = $query->orderBy('lead_id', 'desc')->get(['mul_proid', 'lead_id', 'lead_create_date', 'l_pc', 'fdate', 'l_fname', 'l_lname', 'l_pno', 'l_employee', 'r_employee', 'l_rs', 'source', 'ibpid','l_prodt', 'l_pcfn', 'Property', 'l_rsl', 'ltype', 'ctype', 'l_email', 'fll_details', 'lead_id', 'l_rs_max','property_title','project_society_name'])->toArray();
            // }
            #ssdd($data);
            ...
            foreach ($resultArray as $key => $val) {
                ...
                $ibpid = DB::table('register')->where(['id' => $val['ibpid']])->pluck('name')->toArray();
    ...
    //             $propose_arr = DB::table('temp_lead_cp_app')->where('id', $request->centerid)->update(['site_visit_date' => $site_visit_date, 'site_visit_time' => $request->site_visit_time, 'expiry' => $expiry,'revisit_count' => $count_visit]);
    ...
    //         if ($request->centerid != '') {
    //             $lead_status      = 'LOST';
    //             $propose_arr = DB::table('temp_lead_cp_app')->where('id', $request->centerid)->update(['lead_status' => $lead_status]);
    //             if (isset($propose_arr) && !empty($propose_arr)) {
