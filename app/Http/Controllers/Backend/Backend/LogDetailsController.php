    ...
    public function view(Request $request){
        ...
        if($user == 1){
           ...
           $query = $query->where('mul_proid','=',$proid);
            ...
            $data = $query->get(['l_bid','l_jtitle','l_company','l_tfd','l_itos','l_rs','l_rsl','l_hqc','l_lhs','l_hc','l_pcln','l_pce','l_pcn','l_pcn1','l_pcn2','l_pcn3','l_bid','l_jtitle','purpose','lead_upload_date','ibpid','l_mobile','l_mobile1','l_mobile2','uploadcomment','eadd','adv','prof','id','lead_create_date','l_pc','fdate','l_fname','l_lname','l_pno','l_employee','r_employee','l_rs','source','ibpid','vendor','l_prodt','l_pcfn','Property','l_rsl','ltype','ctype','l_email','fll_details'])->toArray();
			#dd($data);
        ...
        else
        {
         ...
         #dd($insert_log_reason);
            ...
            $leadLog = DB::table('lead_log')->where(['ll_leadid'=>$request->lead_id])->where('mul_proid', '!=', '')->get(['id','ll_details','ll_fillby','mul_proid','mul_proname','ll_username','ll_leadid','status','Date','Time','dtime','ltype','ctype','csm_id','sm_id','followup_medium','lead_log_count','mydate','dead_reason','filename1']);
