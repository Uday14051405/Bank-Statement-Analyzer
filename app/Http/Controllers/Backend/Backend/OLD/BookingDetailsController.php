        ...
        if($user == 1){
           ...
           #dd($da);
            ...
            foreach($resultArray as $key=>$val){
                ...
                $l_employee = DB::table('headcenter')->where(['id'=>$val['l_employee']])->get(['iname','iadd','ihand'])->toArray();
                $r_employee = DB::table('headcenter')->where(['id'=>$val['r_employee']])->get(['iname','id'])->toArray();
                $ibpid = DB::table('ibp')->where(['id'=>$val['ibpid']])->pluck('iname')->toArray();

                $user = DB::table('user')->where(['centerid'=>$r_employee[0]['id']])->get(['name','doj'])->toArray();
                

                $val['l_employee_iname'] = (count($l_employee) > 0 ) ? $l_employee[0]['iname']:'';
                $val['l_employee_iadd']  = (count($l_employee) > 0 ) ? $l_employee[0]['iadd']:'';
                #$val['l_employee_ihand'] = (count($l_employee) > 0 && isset($l_employee[0]['ihand']) ) ? $l_employee[0]['ihand']:'';


                $val['r_employee'] = (count($r_employee) > 0 ) ? $r_employee[0]['iname']:'';
