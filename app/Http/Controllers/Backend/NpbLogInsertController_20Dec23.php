    ...
    public function log_insert(Request $request){
        ...
        if($user == 1)
		{
           /* echo "<pre>";
            ...
            $res_lead_multi = DB::table('npb_lead_create_multi')->where(['lead_id'=>$lead_id,'mul_proid'=>$mul_proid])->first(['ctype','mul_proname','mul_procode','mul_proid','mul_cp','mul_cpsm','mul_dst','r_employee','stype1','mul_source','mul_vendor']);
            ...
            $res_user1 = DB::table('user')->where(['ibpid'=>$centerid_fill,'type'=>'i'])->first(['id','user_name']);
            ...
            $l_employee = DB::select("select * from npb_lead_log where (ltype = 'Site Visits - Done')  AND ll_leadid='".$leadid."' AND mul_proid='".$mul_proid."'");
            ...
            if(count($l_employee) > 0 )
            {
                ...
                {  
                    ...
                    $leadmulti_update = DB::update("update npb_lead_create_multi set log_update_date = '".$date12."',site_update_date = '".$date12."',stype = 'Site Visits - Done', ltype = '".$ltype."',fDate = '".$Date."', fTime = '".$Time."', fll_details = '".$iadd."', fll_fillby = '".$ivoter."', ctype = '".$ctype."', fsub = '".$sub."', status = 'Y' where lead_id='".$leadid."' AND mul_proid='".$mul_proid."'");
                    
                    $l_schedule = DB::select("select * from npb_lead_visit_schedule where lead_id='".$leadid."' AND mul_proid='".$mul_proid."' AND status != 'Done' order by id DESC limit 1");
        
                    if(count($l_schedule) > 0 )
                    {
                        $l_visitschedule = DB::select("select * from npb_lead_log where (ltype = 'Site Visits - Done')  AND ll_leadid='".$leadid."' AND mul_proid='".$mul_proid."'");
                        if(count($l_visitschedule) == 0 )
                        {
                            $date12_visit = date('Y-m-d');
	                        $date12t_visit = date(' H:i');
                            $hla_presales1 = DB::insert("INSERT INTO npb_lead_visit_schedule(lead_id, mul_proid, mul_proname, mul_procode, lead_id_mul, mul_source, mul_vendor, site_visit_date, site_visit_time, status, username, userid, insert_date1, mul_cp, mul_cpsm, mul_dst) VALUES ('".$leadid."','".$mul_proid."','".$mul_proname."','".$mul_procode."','".$leadid."','".$mul_source."','".$mul_vendor."','".$date12_visit."','".$date12t_visit."','Done','".$user_table_name."','".$user_table_id."','".$date12."','".$mul_cp."','".$mul_cpsm."','".$mul_dst."')");
                        }
                        else
                        {
                        ...
                        else
                        {
                             $date12_visit = date('Y-m-d');
                            $date12t_visit = date(' H:i');
                            $leadmulti_update = DB::update("update npb_lead_visit_schedule set site_visit_date = '".$date12_visit."',site_visit_time = '".$date12t_visit."',update_date1 = '".$date12."',update_userid = '".$user_table_id."', update_username = '".$user_table_name."',status = 'Done' where lead_id='".$leadid."' AND mul_proid='".$mul_proid."' AND status != 'Done' order by id DESC limit 1");
...
elseif($ltype == 'Site Visit Schedule')
	{  
        ...
        $leadmulti_update = DB::update("update npb_lead_create_multi set log_update_date = '".$date12."',fDate = '".$Date."', fTime = '".$Time."', fll_details = '".$iadd."', fll_fillby = '".$ivoter."', ltype = '".$ltype."', ctype = '".$ctype."', fsub = '".$sub."', status = 'N',dead_update_date = '".$date12."' where lead_id='".$leadid."' AND mul_proid='".$mul_proid."'");     
    ...
    else
 {
    ...
    $leadmulti_update = DB::update("update npb_lead_create_multi set log_update_date = '".$date12."',fDate = '".$Date."', fTime = '".$Time."', ltype = '".$ltype."', ctype = '".$ctype."', fll_details = '".$iadd."', fll_fillby = '".$ivoter."', fsub = '".$sub."', status = 'Y' where lead_id='".$leadid."' AND mul_proid='".$mul_proid."'");     
...
if($mul_ctype != 'Sale')
{
  if($ctype == 'Sale')
	{  
        ...
        $leadmulti_update = DB::update("update npb_lead_create_multi set sale_update_date = '".$date12."',stype1 = 'Site Visits - Done'  where lead_id='".$leadid."' AND mul_proid='".$mul_proid."'");     
...
if($ltype_re == 'Revisit - 1')
			{  
                ...
                $l_schedule = DB::select("select * from npb_lead_log where (ltype = 'Revisit - 1')  AND ll_leadid='".$leadid."' AND mul_proid='".$mul_proid."'");
...
if($ltype_re == 'Revisit - 2')
			{  
                 ...
                 $l_schedule = DB::select("select * from npb_lead_log where (ltype = 'Revisit - 2')  AND ll_leadid='".$leadid."' AND mul_proid='".$mul_proid."'");
			...
			{  
                ...
                $date12t_visit = date(' H:i');
                 ...
                 $l_schedule = DB::select("select * from npb_lead_log where (ltype = 'Revisit - 3')  AND ll_leadid='".$leadid."' AND mul_proid='".$mul_proid."'");
        ...
        $lead_log_countquery = DB::select("select count(id) as lead_log_count from npb_lead_log where ll_leadid='".$leadid."' and mul_proid='".$mul_proid."' and ll_fillby='".$ivoter."' and lead_log_count_status='1'");
        ...
        $leadmulti_update = DB::update("update npb_lead_create_multi set lead_log_count = '".$lead_log_count_full."',active_date = '".$date12."',active_userid = '".$user_table_id."',active_username = '".$user_table_name."' where lead_id='".$leadid."' AND mul_proid='".$mul_proid."'");
