...
<div class="page-header">
    <div class="card breadcrumb-card">
        <div class="row justify-content-between align-content-between" style="height: 100%;">
            <div class="col-md-6">
                <h3 class="page-title">{{__('deal.my_investment.title')}}</h3>
                <ul class="breadcrumb">
                    <li class="breadcrumb-item">
                        <a href="{{ route('dashboard') }}">Dashboard</a>
                    </li>
                    <li class="breadcrumb-item active-breadcrumb">
                        <a href="{{ route('deal.index_invested') }}">{{ __('deal.my_investment.title') }}</a>
                    </li>
                </ul>
            </div>
                ...
                <table class="table table-hover table-center mb-0" id="table">
                    <thead>
                        <tr>
                            ...
                            <th class="">{{ __('default.table.payment_date') }}</th>
...
$(document).ready(function() {
    ...
    $(document).on('change', '.select-bank-name', function() {
        ...
        $('#confirmButton').off('click').on('click', function() {
            $('#customModal').hide();
            $.ajax({
                url: '{{ route("deal.updatebank", ":id") }}'.replace(':id', dealId),
                type: 'POST',
                data: {
                    bank_id: bankId,
                    _token: '{{ csrf_token() }}'
