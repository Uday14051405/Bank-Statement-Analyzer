...
<body>
    ...
    <p>
        This Agreement for Sales and purchase of receivables (“<strong>Agreement</strong>”) is made between the following on {{ $other_data['formattedDate'] ?? '' }} day of {{ $other_data['monthYear'] ?? '' }}:
    </p>
    <p>
        <!-- <strong>{{ $other_data['vendor_name'] ?? '' }}</strong>, <strong>{{ $other_data['vendor_address'] ?? '' }}</strong>   (ies)-->
        <strong>{{ $other_data['vendor_name'] ?? '' }}</strong></strong> a company that present their invoices for the purpose of sales of receivables(hereinafter referred to as “<strong>Vendor</strong>” or “” which expression shall, unless repugnant to the context or meaning thereof, be deemed to mean and include its successors, related companies and permitted assigns) of the <strong>First Part</strong>;
    </p>
    <p>
        <strong>AND</strong>
    </p>
    <p>
        <strong>{{ $companyDetail->company_name ?? '' }}</strong>, a company incorporated under the Companies Act, [2013], having CIN {{ $companyDetail->cin ?? '' }}, with address {{ $companyDetail->company_address ?? '' }}, {{ $companyDetail->company_city ?? '' }}, {{ $companyDetail->company_state ?? '' }} {{ $companyDetail->company_pincode ?? '' }} (hereinafter referred to as “<strong>Company</strong>” or “<strong>platform</strong>” or “<strong>HEPL</strong>” which expression shall, unless repugnant to the context or meaning thereof, be deemed to mean and include its successors and permitted assigns) of the <strong>Second Part</strong>.
    </p>
    <p>
        <strong>AND</strong>
    </p>
    <p>
        <strong>{{ $other_data['financier_name'] ?? '' }}</strong>, having PAN <strong>{{ $other_data['panNumber'] ?? '' }}</strong> (hereinafter referred to as “Financier”, which expression shall, unless repugnant to the context or meaning thereof, be deemed to mean and include its successors and permitted assigns) of the Third Part.
    </p>
    <p>
    ...
    <div class="clause">
        ...
        <div class="indent">
            ...
            <p class="indent">“Business Day” means any day on which banks are open for normal banking transactions excluding Saturdays, Sundays and any day which is a public holiday in India.</p>
        </div>

        <div class="indent">
            <p class="number">1.1.4</p>
            <p class="indent">“Due Date” is the date on or before which the payment against the relevant Invoices is payable by the Anchor, details of which is more particularly provided in <strong>Annexure 1</strong>.</p>
        </div>

        <div class="indent">
            <p class="number">1.1.5</p>
            <p class="indent">“Designated Account” means the Bank account registered by user, during registering on the Rumbble platform and notified to HEPL through the Platform. </p>
        </div>

        <div class="indent">
            <p class="number">1.1.6</p>
            <p class="indent">“Financier Account” means the bank account of the Financier as registered in the records of HEPL.</p>
        </div>
        ...
        <div class="indent">
            ...
            <p class="indent">“Platform” means the electronic platform created by HEPL under the brand name “Rumbble” to provide online services, available at “[]”.</p>
        </div>
        ...
        <div class="indent">
            ...
            <p class="indent">“Vendor Account” means the bank account of each Vendor as registered in the records of HEPL.</p>
        </div>

        <div class="indent">
            <p class="number">1.1.14</p>
            <p class="indent">“Vendor Facilitation Agreement” means the service level facilitation agreement executed between the Vendor and HEPL.</p>
        </div>

        <div class="indent">
            <p class="number">1.1.15</p>
            <p class="indent">“Vendor Facilitation Fee” means the amount payable to HEPL for facilitating the transaction, as agreed between HEPL and the Vendor.</p>
        </div>
    ...
    <div class="indent">
        <p class="number">2. CONSIDERATION; PAYMENT FLOW AND CONSEQUENCES</p>
        <div class="indent">
    ...
    <div class="indent">
        <p class="number">3. REPRESENTATION AND WARRANTY</p>
        <div class="indent">
    ...
    <div class="indent">
        <p class="number">4. COST; INDEMNIFICATION AND LIMITATION</p>
        <div class="indent">
    ...
    <div class="clause">
        ...
        <div class="indent">
            ...
            <p class="indent"><b>Email: </b>{{ $other_data['email'] ?? '' }}</p>
            <br>
            ...
            <p class="indent"><b>Name: </b>{{ $companyDetail->name ?? '' }}</p>
            <p class="indent"><b>Designation: </b>{{ $companyDetail->designation ?? '' }}</p>
            <p class="indent"><b>Address: </b>{{ $companyDetail->address ?? '' }}</p>
            <p class="indent"><b>Email: </b>{{ $companyDetail->email ?? '' }}</p>
        </div>
    ...
    <div class="clause">
        ...
        <p><strong>Account Name - {{ $companyDetail->company_name ?? '' }}</strong></p>
        <p><strong>Account number - {{ $companyDetail->account_number ?? '' }}</strong></p>
        <p><strong>IFSC Code - {{ $companyDetail->ifsc_code ?? '' }}</strong></p>
        <p><strong>Bank Name - {{ $companyDetail->bank_name ?? '' }}</strong></p>
        <p><strong>Branch - {{ $companyDetail->branch ?? '' }}</strong></p>
    </div>
    <br>
    ...
    <p>SD/- -</p>
    <p>Mr./ MS.: {{ $other_data['vendor_name'] ?? '' }}</p>
    <br><br>
    <p>SIGNED AND DELIVERED</p>
    <p>BY THE WITHINNAMED <strong>“Financier”</strong></p>
    <p>BY THE HAND OF its Authorized Signatory</p>
    <p>SD/- -</p>
    <p>Mr./ MS.: {{ $other_data['financier_name'] ?? '' }}</p>


    <div class="button-container">
        @if ($pdfDownload == 'Yes')
        <form action="{{ route('invest.download-pdf', ['id' => $deal_id]) }}" method="POST">
            @csrf
            <input type="hidden" name="vendor_name" value="{{ $other_data['vendor_name'] ?? '' }}">
            <input type="hidden" name="vendor_address" value="{{ $other_data['vendor_address'] ?? '' }}">
            <input type="hidden" name="financier_name" value="{{ $other_data['financier_name'] ?? '' }}">
            <input type="hidden" name="financier_address" value="{{ $other_data['financier_address'] ?? '' }}">
