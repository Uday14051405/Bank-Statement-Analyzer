            ...
            <div class="container container12 full-cont">
                ...
                <center>
                    <div class="step3 clearfix" style="width: 80%;" id="run_step3">
                        ...
                        <button class="action-btn" id="download-output" {{ $runAnalysisStatus ? '' : 'disabled' }}>Download Excel</button>
                    </div>
                </center>
...
<script>
    ...
    const runAnalysisStatus = @json($runAnalysisStatus);
    ...
    function getBankOptions(selectedBank) {
        ...
        <option value="${bank.code}" ${selectedBank === bank.code ? 'selected' : ''}>
            ${bank.name}
            ${bank.Supported ? `<img src='{{ asset("assets_front/img/Items.png") }}' alt='Logo'>` : ''}
        </option>
    `).join('');
    }
    ...
    <td colspan="5" style="padding: 0; border: 1px solid #dfe0eb; border-top: none; border-radius: 0 0 8px 8px; display: block; margin-bottom: 10px; width: 100%;">
        <!-- Controls Section -->
        <div class="file-controls" style="padding: 15px; background-color:#f5f7fa; border-radius: 0 0 8px 8px; display: flex; flex-direction: column; gap: 10px; width: 100%;">
            
            <!-- First Row: Bank Dropdown & Employer Name -->
            <div class="first-row" style="display: flex; width: 100%; gap: 10px; flex-wrap: wrap;">
                <div class="custom-dropdown" style="flex: 1; min-width: 250px;">
                    <div class="selected-option ${upload_status==='s3' ? 'disabled-div-row' : ''}" data-value="${upload_status==='s3' ? bank_name : 'Select Bank'}" data-flag="${upload_status==='s3' ? pro_version : 'false'}" id="selectedBank_${document_id}" style="background: ${upload_status==='s3' ? '#edeef0' : '#fff'}; pointer-events: ${upload_status==='s3' ? 'none' : 'auto'}; width: 100%;">
                        ${upload_status==='s3' ? full_bank_name.name : 'Select Bank'}
                    </div>
                    <ul class="dropdown-options" id="bankOptions_${document_id}" style="padding-left: 0px; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05) !important; transition: border-color .15s ease-in-out, box-shadow .15s ease-in-out; max-height: 200px; overflow-y: auto; width: 100%;">
                        ...
                        ${banks.map(bank => `
                            <li class="dropdown-option" data-value="${bank.code}" data-flag="${bank.Supported}" data-bnk_nm="${bank.name}" style="width: 100%;">
                                <div class="bank-item">
                                    <span class="bank-name" style="color: ${bank.Supported ? '#000' : '#a6a6a6'}">${bank.name}</span>
                                    ${!bank.Supported ? `
                                        <div class="bank-logo-container">
                                            <span class="only-on-text">only on</span>
                                            <img src='{{ asset("assets_front/img/Items.png") }}' alt='Logo' class="bank-logo">
                                        </div>
                                    ` : ''}
                                </div>
                            </li>
                ...
                <input type="text" class="form-input employer-name ${upload_status==='s3' ? 'disabled-div-row' : ''}" ${upload_status==='s3' ? 'disabled' : ''} placeholder="Employer Name" style="flex: 1; min-width: 250px; padding: 8px; border: 1px solid #ccc; border-radius: 4px; background: ${upload_status==='s3' ? '#edeef0' : '#fff'}" value="${employer_name || ''}">
            </div>

            <!-- Second Row: Password Selection, Password Input, Buttons -->
            <div class="second-row" style="display: flex; width: 100%; gap: 10px; flex-wrap: wrap;">
                <select class="form-select password-select ${upload_status==='s3' ? 'disabled-div-row' : ''}" ${upload_status==='s3' ? 'disabled' : ''} style="flex: 1; min-width: 200px; background: ${upload_status==='s3' ? '#edeef0' : '#fff'}">
                ...
                <input type="password" placeholder="Password" ${upload_status==='s3' ? 'disabled' : ''} style="flex: 1; min-width: 200px; background: ${upload_status==='s3' ? '#edeef0' : '#fff'}" class="form-input enter-pass ${upload_status==='s3' ? 'disabled-div-row' : ''}" value="${file_pwd || ''}" ${!file_pwd ? 'readonly' : ''}>
                
                <button class="edit-btn" data-original-text="Edit" style="min-width: 100px; display: ${upload_status==='s3' ? 'block' : 'none'};">Edit</button>
                <button class="save-btn make-center" style="flex: 1; min-width: 150px; display: ${upload_status==='s3' ? 'none' : 'block'};" data-original-text="Save">Save</button>
            </div>
        </div>
    </td>
`;
    ...
    const setupRowEventListeners = (fileCardRow, controlsRow, fileData) => {
        ...
        saveBtn.addEventListener('click', () => {
            ...
            const emp_name = controlsRow.querySelector('.employer-name').value;
        ...
        editBtn.addEventListener('click', () => {
            ...
            const passwordSelect = controlsRow.querySelector('.password-select');
            const passwordInput = controlsRow.querySelector('.enter-pass');
            const empName = controlsRow.querySelector('.employer-name');
            ...
            empName.classList.remove('disabled-div-row');
        });
    ...
    const updateDocument = async (documentId, uniqueFileName, bankName, emp_name, bankFlag, passwordProtected, password, saveBtn, row, nextRow) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                empName.classList.add('disabled-div-row');
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const status = "{{ $status }}"; // Passed from the backend
        const message = "{{ $message }}";
        const rid = "{{ $rid }}"; // Passed from the backend
