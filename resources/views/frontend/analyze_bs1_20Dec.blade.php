...
<script>
                ...
                <div class="file-header" style="padding:10px;">
                    <div class="file-header-left">
                        <div class="checkbox-wrapper" style="align-self: flex-start; position: relative; bottom: 5px; left:10px;">
                            <input type="checkbox" class="checkbox-input chq-bx ${upload_status==='s3' ? (pro_version == 'true' ? 'upgrade-plan' : '') : ''}" ${upload_status==='s3' ? '' : 'disabled'} ${upload_status==='s3' ? (pro_version == 'true' ? '' : 'checked') : ''}>
                        </div>
                        <div class="file-info">
    ...
    const setupRowEventListeners = (fileCardRow, controlsRow, fileData) => {
        ...
        saveBtn.addEventListener('click', () => {
            ...
            if (!selectedValue || selectedValue === "Select Bank") {
    ...
    const updateDocument = async (documentId, uniqueFileName, bankName, bankFlag, passwordProtected, password, saveBtn, row, nextRow) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                hideLoadingModal_msg('error', `Error updating document: ${data.error}`);
            }
        } catch (error) {
