...
<style>
    ...
    opacity: 0.6; /* Makes the div look faded */
   /* filter: grayscale(50%); /* Optional: makes the div look inactive */
}
...
.disabled-div button {
    ...
    background-color: #b6bcd8; /* Light grey background for disabled inputs */
    color: #fff; /* Grey text color for better contrast */
    border-color: #dcdcdc; /* Light grey border */
    cursor: not-allowed; /* Indicate disabled state visually */
}

.disabled-div input[type="checkbox"],
.disabled-div input[type="radio"] {
    opacity: 0.6; /* Slightly faded appearance */
    pointer-events: none; /* Disable interaction */
}

.disabled-div input[disabled],
.disabled-div textarea[disabled],
.disabled-div select[disabled],
.disabled-div button[disabled] {
    background-color: #b6bcd8 !important;
    color: #fff !important;
...
.disabled-div-row {
    pointer-events: none; /* Disables all interactions */
    opacity: 0.8 !important; /* Makes the div look faded */
    /*filter: grayscale(20%);  Optional: makes the div look inactive */
}

.disabled-div-row input,
.disabled-div-row textarea,
.disabled-div-row select,
.disabled-div-row button {
    pointer-events: none; /* Disables user interactions */
    background-color: #f5f5f5; /* Light grey background for disabled inputs */
    color: #a9a9a9; /* Grey text color for better contrast */
    border-color: #dcdcdc; /* Light grey border */
    cursor: not-allowed; /* Indicate disabled state visually */
}

.disabled-div-row input[type="checkbox"],
.disabled-div-row input[type="radio"] {
    opacity: 0.6; /* Slightly faded appearance */
    pointer-events: none; /* Disable interaction */
}

.disabled-div-row input[disabled],
.disabled-div-row textarea[disabled],
.disabled-div-row select[disabled],
.disabled-div-row button[disabled] {
            ...
            <div class="container container12 full-cont">
                    ...
                    <div class="step {{ $runAnalysisStatus ? 'disabled-div' : '' }}" style="width: 80%;" id="run_step1">
                        ...
                        <div class="step-header-container">
                            ...
                            <div class="step-header-right" style="display: none;">
                                <label id="select-other-img" class="upload-btn" style="float: inline-end; padding: 6px 5px; font-size: 14px;">
                ...
                <center>
                    <div class="step3 clearfix" style="width: 80%;" id="run_step3">
                        ...
                        <button class="action-btn" id="download-output" {{ $runAnalysisStatus1 ? '' : 'disabled' }}>Download Excel</button>
                    </div>
                </center>
        ...
        <div class="loading-modal" id="loadingModal">
            <div class="modal-content">
                ...
                <div class="modal-text">
                    ...
                    <p>You'll be notified via SMS with a download link once it's ready.</p>
                </div>
...
<script>
            ...
            <div class="file-header" style="padding:10px;">
                <div class="file-header-left">
                    <div class="checkbox-wrapper" style="align-self: flex-start; position: relative; bottom: 5px; left:10px;">
                        <input type="checkbox" class="checkbox-input chq-bx" ${upload_status==='s3' ? '' : 'disabled'} ${upload_status==='s3' ? 'checked' : ''}>
                    </div>
                    <div class="file-info">
                        <div>
                        ...
                        <div>
                            ...
                            <div class="file-info-value">${file_name}</div>
                        </div>
                        <div>
                        ...
                        <div>
                            <div class="file-info-label">Max Size (10mb)</div>
    ...
    `;
        ...
        <td colspan="5" style="padding: 0; border: 1px solid #dfe0eb; border-top: none; border-radius: 0 0 8px 8px; display: block; margin-bottom: 10px;">
            ...
            <div class="file-controls" style="padding: 15px 20px; background-color:#f5f7fa; border-radius: 0 0 8px 8px;">
                <select class="form-select bank-select" ${upload_status==='s3' ? 'disabled' : ''} style="background: ${upload_status==='s3' ? '#edeef0' : '#fff'}">
                    <option value="" selected hidden>Select Bank</option>
                    ${getBankOptions(bank_name)}
                </select>
                
                <select class="form-select password-select" ${upload_status==='s3' ? 'disabled' : ''} style="background: ${upload_status==='s3' ? '#edeef0' : '#fff'}">
                ...
                <input type="password" ${upload_status==='s3' ? 'disabled' : ''} style="background: ${upload_status==='s3' ? '#edeef0' : '#fff'}" class="form-input enter-pass" value="${file_pwd || ''}" ${!file_pwd ? 'readonly' : ''}>
    ...
    const setupRowEventListeners = (fileCardRow, controlsRow, fileData) => {
        ...
        saveBtn.addEventListener('click', () => {
            const bankSelect = controlsRow.querySelector('.bank-select').value;
            const passwordProtected = passwordSelect.value;
            const password = passwordInput.value.trim();

            if (!bankSelect) {
                showErrorNotification("Please select a bank.", 'error');
            ...
            if (!passwordProtected) {
                ...
                showErrorNotification("Please select if file is password protected or not.", 'error');
                return;
            }

            if (passwordProtected === "Yes" && !password) {
                //alert("Please enter the password as the file is password protected.");
                showErrorNotification("Please enter the password as the file is password protected.", 'error');
                return;
            }
        ...
        editBtn.addEventListener('click', () => {
            ...
            const bankSelect = controlsRow.querySelector('.bank-select');
            const passwordSelect = controlsRow.querySelector('.password-select');
            const passwordInput = controlsRow.querySelector('.enter-pass');
            bankSelect.style.background = '#fff';
    ...
    const updateDocument = async (documentId, uniqueFileName, bankName, passwordProtected, password, saveBtn, row, nextRow) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                showErrorNotification("Document updated successfully", 'success');
                ...
                saveBtn.style.opacity = "1";
                showErrorNotification(`${data.error}`, 'error');
            ...
            saveBtn.disabled = false;
            saveBtn.style.opacity = "1";
           showErrorNotification(`Error updating document`, 'error');
...
function updateNotificationStatus(rid, status) {
    ...
    fetch(apiUrl, {
        ...
        body: JSON.stringify({ rid: rid, notification_status: status })
    })
